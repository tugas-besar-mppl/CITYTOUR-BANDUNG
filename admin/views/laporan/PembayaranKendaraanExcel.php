...
header('Content-Disposition: attachment; filename="Laporan Pembayaran Jenis Kendaraan.xls"');
...
<table width="100%" style="border:2" class="display" id="gridData">
  <thead>
  <tr>
    <th  align="center" colspan="5"><h3>Laporan Pembayaran</h3></th>    
  </tr> 
  <tr>
  	<th  align="center" colspan="5"><h4><?php echo "Dari Tanggal ".$_GET['tgl']." Sampai Dengan ".$_GET['tgl1'];?></h4></th>
  </tr>  
  <tr>
    <th width="100">No</th>   
...
$query="SELECT
  ...
  m_kendaraan.Jenis,
  m_kendaraan.Harga,  
  SUM(m_kendaraan.Harga * tb_booking.JmlHari) AS THarga,  
  m_wisata.Paket,
  m_pelanggan.Nama,
  tb_booking.JmlHari,
...
FROM tb_booking
  ...
  INNER JOIN m_pelanggan
    ON tb_booking.IdPelanggan = m_pelanggan.IdPelanggan
        WHERE tb_booking.TanggalRegistrasi >=' ".$dateStart."' AND tb_booking.TanggalRegistrasi <=' ".$dateEnd." ' GROUP BY  m_kendaraan.Jenis";
...
?>
  <tr class="list calibriTabel" align="center">
    ...
    <td><?php echo $rows->JmlHari;?></td>             
    <td><?php echo number_format($rows->THarga);?></td>
    
  </tr>
  
<?php 
	$total_penjualan = $total_penjualan  + $rows->THarga;
	$no++;} ?>  
    </tbody>
    <tfoot>
    <tr>
