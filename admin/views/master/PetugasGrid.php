...
"oTableTools": {
	  ...
	  "aButtons": [
				  ...
				  {				
					jQuery("#dialog-form").dialog({
						  ...
						  open: function(event, ui) {
							  jQuery.get("views/master/PetugasForm.php?olah=add", function(data,status) {
								  if (status == "success") {	
					...
					jQuery( "#dialog-form" ).dialog( "option", "title", "Tambah Data Petugas" );
		...
		"sAjaxSource": "models/master/PetugasModels.php",
...
function edit_data(id) {
 jQuery("#dialog-form").dialog({
	...
	buttons: {
	 ...
	 open: function(event, ui) {
		jQuery.get("views/master/PetugasForm.php?olah=edit&key="+id, function(data,status) {
				  if (status == "success") {	
					  jQuery("div#dialog-form").html(data);																	
				  }																									
		}); // end get
    ...
    jQuery( "#dialog-form" ).dialog( "option", "title", "Ubah Data Petugas" );
	jQuery('#dialog-form').dialog('open');  								
}
function hapus_data(id,nama) {
  jQuery("#dialog-form").dialog({
	  ...
	  buttons: {
		 ...
		 open: function(event, ui) {
				jQuery.get("views/master/PetugasForm.php?olah=del&key=id_petugas&val="+id+"&v="+nama, function(data,status) {
				if (status == "success") {	
				   jQuery("div#dialog-form").html(data);
				}
				}); // end get
   ...
   jQuery( "#dialog-form" ).dialog( "option", "title", "Hapus Data Petugas" );
	jQuery('#dialog-form').dialog('open'); 
}
...
<thead>
  <tr>
    <th width="63">NO.</th>
    <th width="159">NAMA</th>
    <th width="239">ALAMAT</th>
    <th width="110">NO. TELPON</th>
