...
"oTableTools": {
	  ...
	  "aButtons": [
				  ...
				  {				
					jQuery("#dialog-form").dialog({
						  ...
						  open: function(event, ui) {
							  jQuery.get("views/system/UserForm.php?olah=add", function(data,status) {
								  if (status == "success") {	
		...
		"sAjaxSource": "models/system/UserModels.php",
...
function edit_data(id) {
 jQuery("#dialog-form").dialog({
	...
	buttons: {
	 ...
	 open: function(event, ui) {
		jQuery.get("views/system/UserForm.php?olah=edit&key="+id, function(data,status) {
				  if (status == "success") {	
					  jQuery("div#dialog-form").html(data);																	
				  }																									
		}); // end get
  ...
  jQuery("#dialog-form").dialog({
	  ...
	  buttons: {
		 ...
		 open: function(event, ui) {
				...
				jQuery.get("views/system/UserForm.php?olah=del&key=id_user_login&val="+id+"&v="+nama, function(data,status) {
				if (status == "success") {	
				   jQuery("div#dialog-form").html(data);
				}
				}); // end get
...
<thead>
  <tr>
    ...
    <th width="239">PETUGAS</th>
    <th width="108">GROUP</th>
