  ...
  $aColumns = array("NO","About","Tanggal","IdAbout" );
  $aColumnsType = array("string","string","string","string");						 
		
  $strQuery = "SELECT  @curRow := @curRow + 1 AS NO, IdAbout,About, DATE_FORMAT(Tanggal,'%d-%m-%Y %H:%i:%s') as Tanggal FROM tb_about  
					  JOIN (SELECT @curRow := 0) AS rNO ";
					 
  $strQueryCount = "SELECT COUNT(IdAbout) FROM tb_about ";
