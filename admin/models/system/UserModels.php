  ...
  $aColumns = array("NO","Username","Nama","NamaGroup","StatusAktif","IdUserLogin" );
  $aColumnsType = array("string","string","string","string","string","string","string" );						 
		
  $strQuery = "SELECT
					  @curRow := @curRow + 1 AS NO,ul.IdUserLogin, ul.Username,  mp.Nama, ug.NamaGroup, ul.StatusAktif
						FROM user_login ul
						  INNER JOIN user_group ug
							ON (ul.IdGroup = ug.IdGroup)
						  INNER JOIN m_petugas mp
							ON (ul.IdPetugas = mp.IdPetugas)
							JOIN (SELECT @curRow := 0) AS rNO
				  					WHERE ul.StatusDelete = 'T'  ";
					 
  $strQueryCount = "SELECT COUNT(ul.IdUserLogin) 
  						FROM user_login ul
						  INNER JOIN user_group ug
							ON (ul.IdGroup = ug.IdGroup)
						  INNER JOIN m_petugas mp
