...
function eraseCurrency(num){
	var temp = "";
	var charTemp = "";
	for (var i = 0; i < num.length; i++){
		charTemp = num.charAt(i);
		if ((charTemp == 'R') || (charTemp == 'p') || (charTemp == ' ') || (charTemp == '.') || (charTemp == ',') || (charTemp == '-')){}
		else
			temp = temp + charTemp;
	}
	return temp;
...
function isDataExist(data_for_check, elm){
	var rowCount = document.getElementsByName(elm).length;
	var i;
	for (i=0;i<rowCount;i++){
		if (data_for_check == document.getElementsByName(elm).item(i).value) {
			return true;
		}
	}
	return false;
...
function isDataExistWithElement(data_for_check, element_for_check, id_tabel){
	var rowCount = jQuery('#'+id_tabel+' tr').length;
	var i;
	for (i=0;i<rowCount;i++){
		if ((element_for_check == getText(document.getElementById(id_tabel).rows[i].cells[0])) && (data_for_check == getText(document.getElementById(id_tabel).rows[i].cells[1]))) {
			return true;
		}
	}
	return false;
